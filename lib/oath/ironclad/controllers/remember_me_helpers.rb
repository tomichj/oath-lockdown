    ...
    module Controllers
      module RememberMeHelpers
        ...
        def rememeber_me(user)
          rememberable = Oath::Ironclad::Adapters::RememberMe.new user
          rememberable.remember_me!
          puts "cookie values? #{remember_cookie_values(user).inspect}"
          cookies.signed[remember_key] = remember_cookie_values(user) # this is breaking
        end
        ...
        def forget_cookie_values(user)
          # cookie_values.merge!(user.rememberable_options)
          cookie_values
        end
